...
$systemuserrights= @()
  foreach ($userright in $rightsarray) {
    $acctlist=@()
	$UR_accounts= Get-AccountsWithUserRight $userright[0]
    foreach ($acct in $UR_accounts.Account) { 
        $acctlist += $acct
        }
        $acctlist = $acctlist -join ','
    
    $newright= New-Object psobject
    $newright | add-member -type NoteProperty -Name "UserRight" -Value $userright[1]
    $newright | add-member -type NoteProperty -Name "Users" -Value $acctlist
    $systemuserrights += $newright
...
$groupcsv= @()
 foreach ( $group in $groups) {
  ...
  $groupcsv_mem = New-Object psobject
  $groupcsv_mem | Add-Member -type NoteProperty -Name "Group Name" -Value $groupname
  $groupcsv_mem | Add-Member -type NoteProperty -Name "Members" -Value $grouplist

  $groupcsv += $groupcsv_mem
  }
...
foreach ($service in $services) {
    ...
    $serv_obj= gwmi -class win32_service -property startmode -filter "NAME=$serv_name"
    $serv_item= New-object psobject

    $serv_item | Add-member -type noteproperty -Name "Status" -Value $serv_obj.StartMode
   
    $serv_item | Add-member -type noteproperty -Name "Name" -Value $service.Name
    $serv_item | Add-member -type noteproperty -Name "Description" -Value $service.DisplayName
    #logonname is a pain
	$service_shortname=$service.name
	$service_startname= (gwmi win32_service -Filter "name=`'$service_shortname`'" ).startname
	$serv_item | Add-member -type noteproperty -Name "Log On As" -Value $service_startname
    $servicescsv += $serv_item
}
#************
if  (! (Test-Path "c:\temp\$env:COMPUTERNAME")) {
    mkdir c:\temp\$env:COMPUTERNAME
}

net user | out-file "c:\temp\$env:COMPUTERNAME\users_$env:COMPUTERNAME.txt"
$systemuserrights | Export-Csv -path "c:\temp\$env:COMPUTERNAME\userrights_$env:COMPUTERNAME.csv"
$groupcsv | Export-Csv -path "c:\temp\$env:COMPUTERNAME\usersgroups_$env:COMPUTERNAME.csv"
get-windowsfeature | out-file "c:\temp\$env:COMPUTERNAME\windowsfeatures_$env:COMPUTERNAME.txt"
$servicescsv | Export-Csv -path "c:\temp\$env:COMPUTERNAME\services_$env:COMPUTERNAME.csv"